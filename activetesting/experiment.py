...
class Experiment:
    ...
    def estimate_risks(self):
        ...
        # risk_estimators = list(self.risk_estimators.values())

        # risk_estimators[0].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[1].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[2].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[3].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[4].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[5].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)

        # risk_estimators[6].estimate(
        #         pred, obs, self.acquisition.weights, surr_model,
        #         acquisition_name=self.acquisition_name)
    ...
    def export_data(self):
        ...
        loss_data = dict()
        idxs = None
        for risk in self.risk_estimators:
            if 'Quadrature' in risk:
                # need to filter true loss vals to the remaining_idxs
                # (bc 1 data is always observed)
                loss_data[risk] = self.risk_estimators[risk].quadrature_loss
                idxs = self.risk_estimators[risk].remaining_idxs
            elif 'Combo' in risk:
                if 'Quadrature' in str(self.risk_estimators[risk].risk2):
                    loss_data[risk] = self.risk_estimators[
                        risk].risk2.quadrature_loss
                    if idxs is not None:
                        idxs = self.risk_estimators[risk].risk2.remaining_idxs
        if idxs is not None:
            loss_data['true'] = self.risk_estimators[
                    'TrueRiskEstimator'].true_loss_all_idxs[idxs]
