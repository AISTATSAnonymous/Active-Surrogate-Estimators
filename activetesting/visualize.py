...
class Visualiser:
    ...
    def read_multiple_experiments(self, paths):
        ...
        if results is None:
                raise ValueError('No experiments found in paths.')
    ...
    def check_configs(self):
        from activetesting.utils.data import from_json
    ...
    def plot_log_convergence(
        ...
        for acquisition, risk in acquisition_risks:
            ...
            if with_errors and errors == 'percentiles':
                ...
                std = self.std_errors.loc[acquisition][risk].values * np.sqrt(prefac) / np.sqrt(self.n_runs) * 2
