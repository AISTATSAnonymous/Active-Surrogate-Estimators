...
def figure_toy_data_main(viss, errors='std'):
    """Results for toy data across different dimensions."""
    labels = [LABEL_RANDOM, LURE_LABEL, ASMC_LABEL, POOL_LIMIT_LABEL]

    fig, axs = plt.subplots(
        1, 4, figsize=(textwidth, 0.3*textwidth),
        sharex=False, sharey=True, dpi=200)

    acq_risks = [
        ['RandomAcquisition', 'BiasedRiskEstimator'],
        ['AnySurrogateAcquisitionValue_LinearModel', 'FancyUnbiasedRiskEstimator'],
        # ['AnySurrogateRandomAcquisition_LinearModel', 'FancyUnbiasedRiskEstimator'],
        # ['AnySurrogateDistanceBasedAcquisition_LinearModel', 'FancyUnbiasedRiskEstimator'],
        # ['AnySurrogateRandomAcquisition_LinearModel', 'FullSurrogateASMC'],
        ['AnySurrogateDistanceBasedAcquisition_LinearModel', 'FullSurrogateASMC'],
        ['RandomAcquisition', 'TrueRiskEstimator'],
        # ['AnySurrogateAcquisitionValue_LinearModel', 'FullSurrogateASMC'],
        # ['AnySurrogateRandomAcquisition_LinearModel', 'ConvexComboWithOutUncertainty-FromIS2Quad'],
        # ['AnySurrogateRandomAcquisition_LinearModel', 'ConvexCombo-FromIS2ASMC'],
        # ['RandomAcquisition', 'BayesQuadRiskEstimator'],
        # ['RandomAcquisition', 'BayesQuadPointWiseRiskEstimator'],
    ]
    ...
    for i, (name, vis) in enumerate(viss.items()):
        df = vis.results
        df = df[(df.id == 0) & (df.acquisition == 'TrueLossAcquisition')]
        mean = df.ExactExpectedRiskEstimator.mean()
        std = df.ExactExpectedRiskEstimator.std()
        print(f'Std of ExactExpectedRiskEstimator {std}')

        plot_log_convergence(
            vis, acq_risks, errors=errors, alpha=0.5, fig=fig, ax=axs[i],
            labels=labels,
            colors=colors,
            prefac=1/name**2,
            linestyle=get_style,
            zorder=[0]*(len(labels)-1) + [-1],
            with_errors=True,
            error_fac=2,
            # transform=None,
            # prefac=1/mean**2,
        ...
        axs[i].set_title(r'$D=${}'.format(name), fontsize=label_fs, pad=4)

    for i, ax in enumerate(axs):
        # ax.set_xscale('linear')
        ax.set_xlabel(ACQUISITION_STEP_LABEL, labelpad=2)
        ax.set_xlim(1, 20)
        # ax.set_ylim(ax.get_ylim()[0], 1e5)
        # if i == 0:
            # ax.set_xlabel(ACQUISITION_STEP_LABEL, labelpad=-1)
        # else:
            # ax.set_xlabel(r'N\textsuperscript{\underline{o}} Acq\dots', labelpad=-1)
    axs[0].set_ylim(1e-2, 2e2)
    axs[0].set_ylabel(r'MSE $ /\, D^2$')
    axs[3].legend(
        **new_kwargs, facecolor='white',
        framealpha=.5, frameon=True)
    ...
    plt.savefig(
        f'notebooks/plots/toy_data_main_{errors}.pdf',
        bbox_inches='tight',
        pad_inches=0.02)
...
def figure_missing7_main(vis, errors='std'):
    ...
    axs[0].set_ylim(axs[0].get_ylim()[0], 3e1)
    ...
    plt.savefig(
        f'notebooks/plots/missing7main_{errors}.pdf',
        bbox_inches='tight',
        pad_inches=0.02)


def figure_missing7_acquisitions(vis, errors='std'):
    ...
    acq_risks  = [
        ['RandomAcquisition_NoSave', 'BiasedRiskEstimator'],
        ['SelfSurrogateAcquisitionSurrogateEntropy_Surr', 'FullSurrogateASMC'],
        ['SelfSurrogateAcquisitionSurrogateEntropy_SurrNoSample', 'FullSurrogateASMC'],
    ...
    labels = [
        LABEL_RANDOM,
        ASMC_LABEL + ' Uncertainty',
        ASMC_LABEL + ' Uncertainty (det.)',
        ASMC_LABEL + ' Expected Loss',
        ASMC_LABEL + ' Exp. Loss + Unc.',]
    ...
    plt.subplots_adjust(wspace=0.1, hspace=0.2)
    plt.savefig(
        f'notebooks/plots/missing7_acquisitions_{errors}.pdf',
        bbox_inches='tight',
        pad_inches=0.02)


def figure_toy_data_gaussians_three_new_bq(viss, viss_bq, errors='percentiles', scale='linear'):
    fig, axs = plt.subplots(
        1, 3, figsize=(textwidth, 0.27*textwidth), sharey=False,
        dpi=200
        )

    acq_risks = [
        # ['RandomAcquisition', 'BiasedRiskEstimator'],
        ['RandomAcquisitionSampleFromPDFDirectly', 'ImportanceWeightedRiskEstimatorForPDFs'],
        ['AnySurrogateAcquisitionValueSampleFromPDFDirectly_TorchGaussianDefensive', 'ImportanceWeightedRiskEstimatorForPDFs'],
        # ['AnySurrogateAcquisitionValue_TorchGaussianDefensive', 'FullSurrogateASMC'],
        ['AnySurrogateAcquisitionValue_TorchGaussianDefensive', 'FullSurrogateASMC-Increase'],
        # ['AnySurrogateAcquisitionValue_TorchGaussianNoClip', 'FullSurrogateASMC-Increase'],
        # ['AnySurrogateAcquisitionValue_TorchGaussianDefensive', 'FancyUnbiasedRiskEstimator'],
        # ['AnySurrogateAcquisitionValueSampleFromPDFDirectly_TorchGaussian', 'ImportanceWeightedRiskEstimatorForPDFs'],
        # ['RandomAcquisition', 'TrueRiskEstimator'],

        # ['AnySurrogateAcquisitionValue_TorchGaussianNoClip', 'FullSurrogateASMC'],
        # ['AnySurrogateAcquisitionValue_TorchGaussianNoSample', 'FullSurrogateASMC'],
        # ['AnySurrogateAcquisitionValue_TorchGaussianDefensive', 'ConvexCombo-FromIS2ASMC'],
    ]
    ...
    for i, (name, vis) in enumerate(viss.items()):
        ...
        if name == '5':
            acq_risks[2][0] = 'AnySurrogateAcquisitionValue_TorchGaussianNoClip'
        else:
            acq_risks[2][0] = 'AnySurrogateAcquisitionValue_TorchGaussianDefensive'

        true_val = vis.results['ExactExpectedRiskEstimator'].values[0]
        prefac = 1/true_val**2
        # post_data = pickle.load(open(Path(vis.path) / 'recompute_mc.pkl', 'rb'))
        # squared_error = post_data['squared_error'][:vis.n_runs]
        # if errors == 'percentiles':
        #     median_squared_error = np.median(squared_error)
        #     lower_quantile = np.percentile(squared_error, 25)
        #     upper_quantile = np.percentile(squared_error, 75)
        #     xlims = [1, vis.cfg['experiment']['abort_test_after']]
        #     axs[i].plot(xlims, 2*[prefac * median_squared_error], c='red')
        #     axs[i].plot(xlims, 2*[prefac * median_squared_error], c='red')

        #     axs[i].fill_between(
        #         xlims,
        #         2*[prefac *lower_quantile],
        #         2*[prefac *upper_quantile], lw=0, color='grey', alpha=0.3)
        # else:
        #     raise


        plot_log_convergence(
            vis, acq_risks[:-1], errors=errors, alpha=0.5, fig=fig, ax=axs[i],
            labels=labels,
            colors=colors,
            # prefac=1/name**2,
            # transform=None,
        ...
        # print('iiiteeerraaatiiion', name)
        labels2 = ['BQ']
        colors2 = [pal[0]]
        # acq_risks2 =  [['RandomAcquisitionSampleFromPDFDirectly', 'BayesQuadRiskEstimator'],]
        acq_risks2 =  [['AnySurrogateAcquisitionValue_BQAcquisition', 'BayesQuadRiskEstimator'],]
        ...
        plot_log_convergence(
            viss_bq[int(name)],
            acq_risks2, errors=errors, alpha=0.5, fig=fig, ax=axs[i],
            labels=labels2,
            colors=colors2,
            # prefac=1/name**2,
            # transform=None,
        ...
        idx = 2
        plot_log_convergence(
            vis, acq_risks[idx:], errors=errors, alpha=0.5, fig=fig, ax=axs[i],
            labels=labels[idx:],
            colors=colors[idx:],
            # prefac=1/name**2,
            # transform=None,
    ...
    if errors == 'percentiles':
        axs[1].set_ylim(axs[1].get_ylim()[0], .7e1)
        axs[2].set_ylim(axs[2].get_ylim()[0], .4e1)

    axs[0].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=2)
    axs[1].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=2)
    axs[2].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=2)
    axs[0].set_ylabel('Squared Relative Error')
    if scale == 'linear':
        axs[2].legend(**new_kwargs, loc='upper right', facecolor='white',
            framealpha=.7, frameon=True)
    else:
        axs[2].legend(**new_kwargs, loc='lower left', facecolor='white',
            framealpha=.7, frameon=True)
    # axs[1].set_xlabel(ACQUISITION_STEP_LABEL)
    # axs[1].xaxis.set_label_coords(1.05, -0.3)
    ...
    plt.savefig(
        f'notebooks/plots/toy_data_gaussians_bq_{errors}_{scale}.pdf',
        bbox_inches='tight',
        pad_inches=0.02)



def figure_missing7_appendix(vis, errors='std'):
    ...
    labels = [
        LABEL_RANDOM,
        LURE_LABEL + ' Constant $\pi=g$',
        ASMC_LABEL + ' Constant $\pi=g$',
        ]
    ...
    plot_log_convergence(
        vis, acq_risks, errors=errors, alpha=0.5, fig=fig, ax=axs[1],
        labels=labels,
        colors=colors,
        # prefac=1/name**2,
        # transform=None,
    ...
    plot_log_convergence(
        vis, acq_risks, errors=errors, alpha=0.5, fig=fig, ax=axs[3],
        labels=labels,
        colors=colors,
        # prefac=1/name**2,
        # transform=None,
    ...
    axs[1].set_ylim(axs[1].get_ylim()[0], 5e2)

    # axs[0].legend(**new_kwargs)
    axs[1].legend(**new_kwargs, loc='upper right', facecolor='white',
        framealpha=.7, frameon=True)
    # axs[2].legend(**new_kwargs, loc='upper right', facecolor='white',
        # framealpha=1, frameon=True)
    axs[3].legend(**new_kwargs, loc='upper right', facecolor='white',
        framealpha=.7, frameon=True)
    ...
    # axs[1].set_xlim(1, 200)
    # axs[2].set_xlim(1, 200)
    # axs[2].set_ylim(axs[2].get_ylim()[0], .7e1)
    # axs[0].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=2)
    axs[1].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=3)
    axs[3].set_xlabel(ACQUISITION_STEP_LABEL, labelpad=3)
    ...
    axs[1].plot(plt_schedule, plt_lims, '-', color='grey', alpha=0.1, lw=1, zorder=-10)
    axs[3].plot(plt_schedule, plt_lims, '-', color='grey', alpha=0.1, lw=1, zorder=-10)

    plt.tight_layout()
    # plt.subplots_adjust(wspace=0.1, hspace=0.2)
    plt.savefig(
        f'notebooks/plots/missing7_appendix_{errors}.pdf',
        bbox_inches='tight',
        pad_inches=0.02)
